...
import { fetchTenders } from '@/app/redux/slice/newApplicationSlice';
...
export default function TableData() {
    ...
    const departmentOptions = Array.from(
        new Set(tenders?.map((tender) => tender?.sector_of_activity || 'N/A')) // Handle null/undefined
    );
    ...
    const filteredTenders =
        tenders?.filter((tender) => {
            const matchesSearchTerm =
                ...
                tender?.sector_of_activity?.toLowerCase().includes(searchTerm.toLowerCase());
            const matchesDepartment =
                departmentFilter === 'All' || tender?.sector_of_activity === departmentFilter;
    ...
    if (error) return <p className="text-center">Error: Something went wrong</p>;
                    ...
                    <table className="min-w-full bg-white table-auto border border-gray-200 p-2">
                        <thead>
                            <tr className="bg-gray-100 text-black font-bold text-sm leading-normal">
                                ...
                                <th className="py-3 px-6 text-left">Application Date</th>
                            ...
                            {currentItems.length > 0 ? (
                                currentItems.map((tender, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-3 px-6 text-left">
                                            {tender?.sector_of_activity}
                                        </td>
                                        <td className="py-3 px-6 text-left">
                                            {tender?.created_at
                                                ? new Date(tender.created_at).toLocaleString(
                                                      'en-US',
                                                      {
                                                          year: 'numeric',
                                                          month: 'long',
                                       ...
                                       <td className="py-3 px-6 text-right">
                                        <Link href={`/admin/add-tender?id=${tender?.id}`}>
